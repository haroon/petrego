"""Test cases for the version 2 of the API.
"""
import sys
sys.path.append('..')
...
class Tests(unittest.TestCase):
    ...
    def test_animalfood(self):
        ...
        """POST"""
        res = self.client.post('/api/v2/animalfoods/snake/mice/')
        self.assertEqual(res.status_code, 201)

        """GET"""
        res = self.client.get('/api/v2/animalfoods/snake/')
        result = json.loads(res.data.decode())['data']
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0]['food'], 'mice')

        """PUT"""
        res = self.client.put('/api/v2/animalfoods/snake/rats/')
        self.assertEqual(res.status_code, 201)
    ...
    def test_pets(self):
        ...
        # owner with no pets
        res = self.client.get('/api/v2/pets/f2/l2/')
        self.assertEqual(res.status_code, 200)
        result = json.loads(res.data.decode())
        self.assertEqual(len(result), 0)

        # invalid pet owner
        res = self.client.get('/api/v2/pets/f5/l5/')
        self.assertEqual(res.status_code, 200)
        result = json.loads(res.data.decode())
        self.assertEqual(len(result), 0)
    ...
    def test_owners(self):
        ...
        for owner in owners:
            res = self.client.post('/api/v2/owners/{}/{}/'.format(
                owner[0], owner[1]))
            self.assertEqual(res.status_code, 201)

        """GET"""
        # All available owners
        res = self.client.get('/api/v2/owners/')
        self.assertEqual(res.status_code, 200)
        result = json.loads(res.data.decode())['data']
    ...
    def _prepare_pets(self):
        ...
        for animal_food in animal_foods:
            res = self.client.post('/api/v2/animalfoods/{}/{}/'.format(
        ...
        # Create some pets
        pets = [('p1', 'snake', 'f1', 'l1', 'mice'),
                ('p2', 'chicken', 'f1', 'l1', 'corn'),
                ('p3', 'dog', 'f1', 'l1', 'bones'),
                ('p4', 'cat', 'f1', 'l1', 'fish'),
                ('p5', 'snake', 'f1', 'l1', 'mice')]

        for pet in pets:
            res = self.client.post('/api/v2/pets/{}/{}/{}/{}/'.format(
    ...
    def _verify_pets(self, pets):
        ...
        """
        res = self.client.get('/api/v2/pets/f1/l1/')
        self.assertEqual(res.status_code, 200)
        result = json.loads(res.data.decode())['data']
        ...
        for r, p in zip(result, pets):
            ...
            self.assertEqual(r['food_name'], p[4])
